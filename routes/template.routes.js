...
templateController.post("/save-template",async(req,res)=>
{
    ...
    const {templateName,pollName,questions} = req.body;
    try{
        const template = new TemplateModel({
            adminId:user.userId,templateName,pollName,questions});
           
         await template.save();
            
         const updateUser= await UserModel.findOneAndUpdate({_id:user.userId},{$push:{templateCreated:{templateId:template._id,templateName,pollName}}});
